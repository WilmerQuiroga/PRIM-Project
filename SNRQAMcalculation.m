...
% radio frames required for the transmission of the image data. A
% scaling factor of |scale = 0.5| requires five LTE radio frame transmissions.
% Increasing the scaling factor results in more frame transmissions;
% Reducing the scaling factor reduces the number of frames.
...
sdrTransmitter.IPAddress='137.194.172.35';
...
sdrReceiver.Gain=0;
sdrReceiver.IPAddress='137.194.172.35';
...
j=1;
Attenuation=0:2:60;
SNR_freq=zeros(1,length(Attenuation));

for i=0:2:60
    
sdrTransmitter.Gain=-i;
...
BW=7.6e6;
Fs=15360000;
Spectrum3M=plot_spectrum(rxWaveform,1,Fs,0,0);
fin=2251050;
N=length(rxWaveform);
bin_in=round(fin/Fs*N)+1;
fin=bin_in*Fs/N;
SNR_freq(j)=SNR_F(Spectrum3M,N,fin,Fs,BW);
j=j+1;
end
...
figure(1)
%plot(Attenuation,SNR_freq,'linewidth',2);
plot(Attenuation,SNR_freq,'linewidth',2);
xlabel('Attenuation(dB)')
ylabel('SNR(dB)')
title('SNR vs Attenuation Modulation')
set(gca, 'FontSize', 15);
