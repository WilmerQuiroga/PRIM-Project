...
%%
% we start by defining the common simulation parameters
frmLen = 100;           % frame length
maxNumErrs = 300;       % maximum number of errors
maxNumPackets = 9000;   % maximum number of packets
EbNo = 0:2:14;          % Eb/No varying to 12 dB
...
title(ax,'Alamouti-coded 2x2 System 16-QAM');
...
for idx = 1:length(EbNo)
    ...
    while (ber_Estimate(2,idx) < maxNumErrs) && ...
        ...
        demodEst   = qamdemod(decDataEst,16);      % estimated
        demodKnown = qamdemod(decDataKnown,16);    % known      
